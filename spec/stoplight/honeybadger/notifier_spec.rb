...
RSpec.describe Stoplight::Honeybadger::Notifier do
  it 'is a class' do
  ...
  describe '#options' do
    it 'is initially the default' do
      expect(described_class.new(nil).options).to eql(Stoplight::Honeybadger::Notifier::DEFAULT_OPTIONS)
    end

    it 'reads the options' do
      options = { key: :value }
      expect(described_class.new(nil, nil, options).options)
        .to eql(Stoplight::Honeybadger::Notifier::DEFAULT_OPTIONS.merge(options))
    end
  end
  ...
  describe '#notify' do
    ...
    it 'returns the message' do
      ...
      expect(notifier.notify(light, from_color, to_color, error)).to eql(message)
      expect(Honeybadger).to have_received(:notify).with(hash_including(api_key: api_key, error_message: message))
    end
    ...
    it 'returns the message with an error' do
      ...
      expect(Honeybadger).to have_received(:notify).with(
        hash_including(api_key: api_key, error_message: message, backtrace: error.backtrace)
      )
    end
  end
end
